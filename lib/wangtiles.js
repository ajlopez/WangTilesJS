...
function fillSouth(data, width, offsetx, offsety, size, color) {
    for (var y = 2; y <= size / 2 - 1; y++)
        for (var x = y; x < size - y; x++) {
            var k = ((size - y + offsety) * width + x + offsetx) * 4;
    ...
    this.drawTiles = function () {
        ...
        for (var x = 0; x < width; x++)
            for (var y = 0; y < height; y++) {
                ...
                var offsetx = x * cellsize;
                var offsety = (height - y - 1) * cellsize;
...
function loadPlane(options) {
    var plane = createPlane(options.width, options.height);
    
    if (options.positions)
        for (var n in options.positions) {
            var position = options.positions[n];
            var tile = createTile(position.tile);
